...
router.post("/api/updates", (req, res) => {
  ...
  update.save().then(update => {
    ...
    else res.status(500).json({ errors: { global: "Invalid credentials." } });
  });
});
...
router.delete("/api/updates/:id", (req, res) => {
  Updates.findByIdAndRemove(req.params.id)
    .then(response => {
...
router.put("/api/updates/:id", (req, res) => {
  Updates.findByIdAndUpdate(req.params.id, req.body.update, { new: true })
    .then(update => {
      res.json({ update });
    })
    .catch(error => {
...
router.get("/api/updates/:kidId", (req, res) => {
  Updates.findById({ to: req.params.kidId })
    .then(update => {
      res.json({ update });
    })
    .catch(error => {
